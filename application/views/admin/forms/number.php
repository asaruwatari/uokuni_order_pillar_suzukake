...
<div class="form-group">
    ...
    <div class="field">
        <input type="number" name="<?= h($field) ?>" style="<?= @$unit ? 'display:inline-block; width:10em' : '' ?>" class="form-control form-control-sm <?= h($class) ?> <?= form_error($field) ? 'is-invalid' : '' ?>" value="<?= set_value($field, isset($data[$field]) ? $data[$field] : '') ?>" placeholder="<?= @$guide ?: '' ?>" <?= $readonly ? 'readonly' : '' ?> autocorrect="off" autocapitalize="off">
        <?= @$unit ?: '' ?>
